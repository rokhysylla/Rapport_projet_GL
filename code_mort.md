...
Cette section analyse la présence de code mort dans trois classes clés de Gson : TypeAdapters, GsonTypes et LinkedTreeMap et parseBenchmark. L'objectif est d'identifier les portions de code non exécutées ou rarement utilisées, d'évaluer leur nécessité et de proposer des améliorations en matière de couverture de test. Bien que certaines branches conditionnelles restent peu testées, la plupart du code identifié n'est pas véritablement "mort" mais plutôt destiné à gérer des cas limites, des erreurs ou des configurations spécifiques.



### 5.4 Code mort - TypeAdapters.java

Dans TypeAdapters, je ne vois pas de code mort évident du type "méthode jamais appelée" à l'intérieur du projet, parce que c'est surtout une classe utilitaire avec des constantes publiques. Comme c'est une bibliothèque, c'est normal que certains adapters ne soient pas utilisés par Gson lui-même mais par des projets externes.

Par contre, je peux identifier des morceaux de code qui ne servent qu'à gérer des cas rares. Par exemple, CLASS.read() et CLASS.write() ne font que lever une exception, donc ce code n'est exécuté que si quelqu'un tente de sérialiser/désérialiser Class. De la même manière, toutes les branches qui lancent des JsonSyntaxException (valeurs invalides, token inattendu) sont du "code d'erreur" qui peut rester non appelé dans un usage standard. Vu nos observations précédentes sur la couverture, ce genre de branches est typiquement peu testé.

Je ne pense pas qu'il faille supprimer ces blocs : ils sont là pour sécuriser et guider l'utilisateur (messages d'erreur + troubleshooting). En revanche, c'est un bon candidat pour des tests ciblés "entrées invalides", justement pour éviter que ces chemins restent en rouge dans la couverture.

Bref : pas du code mort à supprimer, plutôt du code rarement exécuté mais utile et à mieux tester.
